...
String formatDateSearch(DateTime date) {
  final DateTime now = DateTime.now();

  return DateFormat('d MMMM, E', 'ru').format(date);
}

class DataCheckSearh extends StatelessWidget {
  final String date;
  final String cash;
  final String totalCash;

  const DataCheckSearh({
    super.key,
    required this.date,
    required this.cash,
    ...
    return Padding(
      ...
      child: Row(
        children: [
          RichText(
              text: TextSpan(
            text: formatDateSearch(formatttedDate), // Получаем первые два символа
            style: const TextStyle(
                fontSize: 16, // Новый размер шрифта для первых двух символов
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              ...
                              if (type == 'Исходящий перевод' || type == 'Покупка по карте') // Проверяем статус чека
                                Padding(
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        if (type == 'Входящий перевод' && icon == 'Т Банк') // Проверяем статус чека
          ClipRRect(
            borderRadius: BorderRadius.circular(20), // Установите радиус, чтобы получить круглую форму
            child: Image.asset(
        ...
        if (type == 'Входящий перевод' && icon == 'МТС Банк') // Проверяем статус чека
          ClipRRect(
            borderRadius: BorderRadius.circular(20), // Установите радиус, чтобы получить круглую форму
            child: Image.asset(
              'assets/banks/мтс.jpg',
              width: 38,
            ),
          ),
        if (type == 'Входящий перевод' && icon == 'Яндекс Банк') // Проверяем статус чека
          ClipRRect(
            borderRadius: BorderRadius.circular(20), // Установите радиус, чтобы получить круглую форму
            child: Image.asset(
              'assets/banks/яндекс.jpg',
              width: 38,
            ),
          ),
        ...
        if (type == 'Покупка по карте') // Проверяем статус чека
          Container(
              padding: const EdgeInsets.all(7),
              decoration: const BoxDecoration(
                borderRadius: BorderRadius.all(Radius.circular(20)),
                color: Color.fromARGB(255, 16, 129, 50),
              ),
